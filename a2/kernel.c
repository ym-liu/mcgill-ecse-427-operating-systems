...
int process_initialize(char *filename)
{
    ...
    PCB *newPCB = makePCB();
    int line_start = 0;
    while (!feof(script_in_backing_store_fp)) // while havent reached eof,
    {                                         // load source code as pages of 3 lines
        int error_code = load_page(script_in_backing_store_fp, filename, newPCB, line_start);
        if (error_code != 0)
        {
    ...
    newPCB->PC = (*(newPCB->page_table))->index[0];
    printShellMemory();
...
bool execute_process(QueueNode *node, int quanta)
{
    ...
    for (int i = 0; i < quanta; i++)
    {
        ...
        if (pcb->PC > (*(pcb->page_table))->index[2]) // pcb->end
        {
            parseInput(line);
